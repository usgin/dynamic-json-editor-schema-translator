	...
	if ( typeof(jSchema.children) != "undefined" )  {
		for (var subkey = 0; subkey < jSchema.children.length; subkey++) {
			...
			if ( typeof(schemaChild.ref) != "undefined" ) {
				    ...
				    /*
					var mdlookup = schemaChild.ref;
				    if ( Array.isArray(mdlookup) ) {
	            		for (var k in mdlookup) {
	            			var ajT;
	            			found="";
	            			var mdarrLookup = jsonLookup(jObjData, ajT,mdlookup[k], true);
	            			console.log(' sub object ref array lookup ' + mdlookup[k] + ' ' + k + ' ' + mdarrLookup);
	            			if (typeof(mdarrLookup) != "undefined" && mdarrLookup != "") {
	            				console.log(' >>> Used this ' + mdlookup[k] + ' ' + k + ' ' + mdarrLookup);
	            				mdlookup= mdlookup[k];
	            				schemaChild.value = mdarrLookup;			
	            				break;	
	            			}
	            			// what to do if nothin found
	            		}
	            	} else {
						var subpath = mdlookup;
						var njT;
						console.log(' level 1 sub object start ' + subpath);
						found = "";
						var newt = jsonLookup(jObjData, njT,subpath, true);
						schemaChild.value = newt;
						console.log(' level 1 sub object ' + subpath + ' : ' + schemaChild.value);
					}
					*/
			}
			...
			if ( schemaChild.children ) {
				for (var sk = 0; sk < schemaChild.children.length; sk++) {
					...
					if ( typeof(subChild.ref) != "undefined" ) {
							...
							/*
							var s2path = subChild.ref;
							if ( Array.isArray(s2path) ) {
			            		for (var k in s2path) {
			            			var aJT;
			            			var mdarrLookup = jsonLookup(jObjData, ajT,s2path[k], true);
			            			if (mdarrLookup) {
			            				s2path= s2path[k];
			            				break;	
			            			}
							...
							var ojT;
							found = "";
							subChild.value = jsonLookup(jObjData, ojT,s2path, true);
							console.log(' level 2 sub object ' + s2path + ' : ' + subChild.value);
							*/
					}
