...
function visu (error, treeData) {
   ...
   // if ( (typeof(errDoc.width() !== "undefined") ) && (typeof(errDoc.width() !== "null" ) ) && (typeof(errDoc.body.scrollWidth() !== "undefined") ) ) {
  ...
  /*  d3.selectAll('.pop-up')
    ...
    function update(source) {
        ...
        nodes.forEach(function(d) {
            ...
            if (typeof(d.parent) !=="undefined") { parY = d.parent.y + 10; }
            if (typeof(d.parent) !=="undefined" && typeof(d.parent.datatype) !=="undefined") {
                ...
                if (d.parent.datatype =="array" || d.parent.datatype =="object" ) {scaleFactor = 4}
                if (d.parent.datatype =="textarea"  ) {scaleFactor = 4}
                if (d.parent.datatype =="date" || d.parent.datatype =="dictlist"  ) {scaleFactor = 4}
                //if (d.parent.name =="contactEmails") {scaleFactor = 1}
            }
            //d.y = parY + scaleFactor*msgLen;
            /*
            if (typeof(d.datatype) !=="undefined" ) {
                //parY = d.parent.y;
                ( typeof(d.name) !== "undefined") ? msgLen = d.name.length: console.log("no name");
                if (d.datatype =="array" || d.parent.datatype =="string" || d.parent.datatype =="date"  ) {scaleFactor = 4}
                if (d.datatype =="textarea"  ) {scaleFactor = 5}
                if (d.datatype =="date" || d.parent.datatype =="dictlist"  ) {scaleFactor = 5}
            }
*/
            if  (d.depth == 1 ) { d.y = (1 + d.depth) * maxLabelLength*scale } // +  msgLen * scaleFactor; // GHud -- maxLabelLength * 10px
            if  (d.depth > 1 ) {
                d.y = parY + maxLabelLength*scaleFactor; }
            if  (d.depth == 0 ) { d.y = maxLabelLength * scale; }
        ...
        var nodeEnter = node.enter().append("g")
            ...
            /* function(d) {
                    if ( typeof(d.datatype) != "undefined" && d.datatype == "array" ) {
                       return d3.contextMenu(arrayMenu)  
                   } else { return d3.contextMenu(menu) } }); */
