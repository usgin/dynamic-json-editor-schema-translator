...
exports.ta_query = function(rscName,params) {
    var CM = [];
    //return CM;

   console.log(' params ' + JSON.stringify(params) );

    if ( params.field == "usginContentModel" ) {  
        for (var itm in contentMod ) { 
                 var mv = itm.split("+");
                 ( !CM.contains(mv[0]) ) ? CM.push(mv[0]) : null;
                 console.log(' model items ' + JSON.stringify(itm));
        };
        return CM;  
        
    }

    if ( params.field == "usginContentModelLayer" ) { 
        var CM = [];
        if ( params.query.length > 0 && params.ref.length > 0 ) {
             for (var itm in contentMod ) { 
                 var mv = itm.split("+");
                 if ( mv[0] == params.ref ) {
                     ( !CM.contains(mv[1]) ) ? CM.push(mv[1]) : null;  
                      console.log(' layer items ' + mv[1]); //JSON.stringify(itm));  
                 } 
          };
        }  else {
            for (var itm in contentMod ) { 
                 ...
                 //if ( mv[0] == params.ref ) {
                 ( !CM.contains(mv[1]) ) ? CM.push(mv[1]) : null;  
                 console.log(' unfiltered layer items ' + mv[1]); //JSON.stringify(itm));  
                 //} 
          };
    ...
    if ( params.field == "usginContentModelVersion" ) { 
        var CM = [];
        if ( params.query.length > 0 && params.ref.length > 0 ) {
             for (var itm in contentMod ) { 
                 var mv = itm.split("+");
                 if ( mv[1] == params.ref ) {
                     var vurl = mv[2].split("/").pop();
                     ( !CM.contains(vurl) ) ? CM.push(vurl) : null;    
                 } 
          };
        }
        return CM;
    ...
    for (var itm in contentMod ) { 
             var mv = itm.split("+");
             var shoV = mv[0]; // + " " + mv[1];
             ( !CM.contains(shoV) ) ? CM.push(shoV) : null;
