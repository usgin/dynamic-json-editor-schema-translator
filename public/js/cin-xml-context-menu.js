...
function newSrc3 (dob, copyKey, action) {
    ...
    if ( dob.SourceRef ) {
        ...
        // Maybe it has a template we can insert
    ...
    if ( saPath && !Array.isArray(saPath) ) { 
      saPath = saPath.split('.');
    }
...
function fpath(lookup,name) {
  ...
  for (a =0;  a < tko.length; a++ ) {
    ...
    for (var i in gFlat) {
        ...
        if ( pn.indexOf('description') != -1 ) {
            console.log(' paths ' + i + ' Flat Path ' + pc + ' -Lookup- ' + tk + ' -Fieldname-' + name + ' -Last Flat- ' + pln);
			var ak = pln.indexOf(name);
			var am = pc.indexOf(tk);
        }
                ...
                for (v =0;  v < eLen; v++ ) {
                    ...
                    if (OrigDoc.hasOwnProperty(elem) ) {
                      if ( v == ( eLen - 1 )  ) {
						...
						if ( OrigDoc[elem] ) {    // It has the top element - attach as attribute this is elem
							  var odx = OrigDoc[elem];
							  odx[otype] = gSchemaStack[otype];
						} else {  // It doesnt exist so create it
							OrigDoc[elem] = gSchemaStack[otype];
						}
                        //var sO = gSchemaStack[otype];
                        //OrigDoc[otype] = gSchemaStack[otype];
...
function upSrc2( dob, value ) {
    ...
    if ( dob.RefSaveTo ) {
        var mPath = dob.RefSaveTo;
         ...
         for (v =0;  v < dob.ref.length; v++ ) {
            ...
            if ( typeof(dob.array_index) !== "undefined" ) {
              tk = dob.parent.RefSaveTo + '.' + dob.array_index + '.' + dob.ref[v];
              var fixpath = fpath(tk,dob.name);
              if ( fixpath.length ) {
                dob.RefSaveTo = fixpath;
                mPath = fixpath;
              ...
              var lazyk = dob.parent.RefSaveTo + '.' + dob.ref[v];
              var fixpath = fpath(lazyk,dob.name);
              if ( fixpath.length ) {
                dob.RefSaveTo = fixpath;
                mPath = fixpath;
                break;
    ...
    // IF we are still bad - can we build a missing schema object from template
    if ( typeof(mPath) == "undefined" ) {
      if ( typeof(dob.pathInsert) != "undefined")  {
         ...
         var otype = dob.insertType;
         if ( typeof(dob.array_index) !== "undefined" ) {
            pip = dob.parent.RefSaveTo + '.' + dob.array_index + '.' + dob.pathInsert;
         }
         var sces = insertSchema(pip, otype);
...
var cloneDbase = function(did,sibs,clr,newParent) {
    ...
    if ( typeof(did.array_index) !== "undefined" ) {
       siboff = 35*(sibs - did.array_index - 1);
    } else { 
        siboff = 32*(sibs-1); 
    } 
    ...
    /*
    if ( typeof(did.array_index) !== "undefined" ) {
       if ( sibs > 3  && did.datatype == 'url' ) {
        siboff = 40*(sibs - did.array_index);
        } else if ( sibs <= 3  && did.datatype == 'url' ) {
           siboff = 25*(sibs - did.array_index);
        } 
        if ( sibs > 10  && did.name == 'keywords' ) {
          siboff = 5*(sibs - did.array_index);
        } else if ( sibs <= 10 && sibs > 3 && did.name == 'keywords' ){
           siboff = 10*(sibs - did.array_index);
         } else if ( sibs <= 3 && did.name == 'keywords' ){
           siboff = 15*(sibs - did.array_index);
        } 
    ...
    */
	  ...
	  if ( did.datatype == 'bbox' ) {
		  siboff = siboff+5;
	  }
	
    //var cXoff = 0;
    if ( typeof(did.xoffset) !== "undefined"  ) {
        var xoffsize = did.xoffset - did.savedX;
    } else {
        var xoffsize = 0;
    }
    
    if ( newParent ) {
     if ( typeof(newParent.xoffset) !== "undefined"  ) {
        var rdiff = newParent.xoffset - newParent.savedX;
        //newClone.xoffset = newParent.savedX + siboff + xoffsize;
         if (  typeof(newParent.btmX) !== "undefined"  ) {
           newClone.xoffset = newParent.btmX +40;
           newClone.btmX = newParent.btmX;
           newParent.btmX = newParent.btmX +40;
           
         } else {
           newClone.xoffset = newParent.offset + 40;
           newParent.btmX = newClone.xoffset;
         }
         
     } else {
         var rdiff = did.xoffset - did.savedX;
         if (  typeof(did.btmX) !== "undefined"  ) {
           newClone.xoffset = did.btmX + 40;
          } else {
           newClone.xoffset = did.savedX + 40;
     ...
     if ( typeof(newParent.xoffset) !== "undefined"  ) {
        ...
        newClone.xoffset = newParent.savedX + siboff;
        
     } else {
         newClone.xoffset = did.savedX + siboff;
     }
     */
      ...
      if (  typeof(did.btmX) !== "undefined"  ) {
         ...
         newClone.xoffset = did.xoffset + siboff + xoffsize;
       }
      
      newClone.array_index = sibs;
    }
    newClone.Cloned = true;
...
var cloneDid = function(did, sibs, clr, newParent) {
	...
	if ( !newParent && did.RefSaveTo && kids ) {
	  ...
	  newClone.RefSaveTo = did.RefSaveTo;
      newClone.SourceRef = baseRef;
        ...
        if (typeof(data.readonly) !== "undefined" && data.readonly == "true") {
            ...
            if (typeof(data.datatype) !== "undefined" && data.datatype == "array") {
                ...
                data.children.forEach(function(d) {
                    if ( d.value ) {
                        if (d.name == "gmd:eastBoundLongitude") { eb = d.value }
                        if (d.name == "gmd:westBoundLongitude") { wb = d.value }
                        if (d.name == "gmd:northBoundLatitude") { nb = d.value }
                        if (d.name == "gmd:southBoundLatitude") { sb = d.value }       
                    }
                });
            }
            ...
            if ( data._children ) {
                ...
                gExt[3] = sb;
                 ...
                 list.selectAll('li').data(menuSel).enter()
                    .append('li')
                    .html(function (d) {
						var shoVal = ( data.value == "Edit" ) ? "" : data.value;
                        var selStr = d.title + '</br><div id="boxmap" class="map"></div>' +
                                        ...
                                        '<input class="typeahead-text" name="contentModel" type="text" placeholder="Place Typeahead" value="' + shoVal + '">' + 
                                        '</div></div>';
                ...
                menuSel = objMenu;
                if ( data.pipelineEnhanced ) {
                  if ( data.validState ) {
                     objMenu[0].title = "Invalidate";
                  } else {
                     objMenu[0].title = "Validate";
                  }
                }
                list.selectAll('li').data(menuSel).enter()
                .append('li')
                  ...
                  list.selectAll('li').data(menuSel).enter()
                        ...
                        .html(function (d) {
							...
							if ( !data.parentURL ) {
							  ...
							  var editStr = d.title + ' <b>' + data.name + '</b>';
							}
                            return ( d.title == 'Edit' ) ? editStr : d.title;
...
var mapMenu = [{ title: 'Bounding Box',
    ...
    {title: 'Save',
      action: function(elm, d, i) {
		...
		if ( d.value != rexedit && rexedit.length > 0 ) { 
	        // Check if extent is present	
			if ( typeof(d.RefSaveTo) == "undefined" ) {
				 upSrc2(d, rexedit);
				/* 
				 if ( (Array.isArray( dob.ref) ) ) {
					for (v =0;  v < dob.ref.length; v++ ) {
						var tk = dob.ref[v];
						var fixpath = fpath(tk,d.name);
						if ( fixpath.length ) { break; }
					}
					// Doesnt exist in mdJSON
					var sces = insertSchema(d.pathInsert, d.insertType);
					if ( sces.length ) {
						d.RefSaveTo = sces;
					}
				 }
              ...
              var btmX = -999;
               ...
               children.forEach(function(child, index) {
                ...
                if (lkey == rexedit && !isDone) {
					...
					copyChild.siblings = copyChild.siblings+1;
