...
function startPipelineProcess() {
  ...
  alert('Not active in demo version');
  
}

var saveByIndexID = function( ) {
...
function saveMdb() {
      // Current Cinergi Save - saves to db
	...
	/* This saves to index - disabled for Docker
      $.ajax({ 
        type: 'POST',
        url: saveURL,
        processData: false,
...
function mongodbSave() {
   ...
   $.ajax({ 
        ...
        success: function(data) {
          ...
          res = data.result;
          var pstack = 'Save Package :' + gTitle; 
          alert(pstack);
          clearEdits();
          listEditHistory();     
...
function search_package(curPage) {
  // Search records not used in Cinergi
...
// This retrieves the index ID and then calls the reindex request (trigIndex)
